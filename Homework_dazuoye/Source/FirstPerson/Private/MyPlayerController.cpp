...
void AMyPlayerController::BeginPlay()
{
	...
	if (FirstPersonHUD!=nullptr)
	{
		...
		FEndGameCallBack Func = std::bind(&AMyPlayerController::LoseGame, this);
		FirstPersonHUD->GetMyUserWidget()->SetFuncPtr(Func);
	}
...
void AMyPlayerController::OnEndGame(FString EndText)
{
	FirstPersonHUD->DestroyMyUserWidget();
	SetPause(true);
	SetInputMode(FInputModeUIOnly());
	...
	FirstPersonHUD->CreateEndGameWidget();
	UEndGameWidget* EndGameWidget = FirstPersonHUD->GetEndGameWidget();
	EndGameWidget->SetEndGameText(EndText);
}

void AMyPlayerController::LoseGame()
{
	OnEndGame(FString("You Lose!"));
}

void AMyPlayerController::WinGame()
{
	OnEndGame(FString("You Win!"));
}
